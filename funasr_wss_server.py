    ...
    if len(audio_in) > 0:
        ...
        if websocket.is_speaker_verification:
            if websocket.speaker_verification_activate:
                signal = bytes_to_tensor(audio_in)
                audio_emb = spkrec.encode_batch(signal).squeeze(0)
                sim = cosine_similarity(audio_emb, websocket.speaker_verification_sample_emb)
                logger.info(f"async_asr cosine_similarity:  {sim}, SPEAKER_VERIFICATION_THRESHOLD: {SPEAKER_VERIFICATION_THRESHOLD}")
                if sim >= SPEAKER_VERIFICATION_THRESHOLD:

...
# 声纹接口参数
voiceprint_api_url = os.getenv("VOICEPRINT_API_URL", "http://127.0.0.1:18005/voiceprint/identify")
voiceprint_api_key = os.getenv("VOICEPRINT_API_KEY")
voiceprint_speaker_ids = ["qiaoyafeng", "guochao"]
...
async def identify_speaker(websocket, audio_data: bytes):
        """识别说话人"""
        ...
        try:
            ...
            # 准备请求头
            headers = {
                'Authorization': f'Bearer {voiceprint_api_key}',
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.post(voiceprint_api_url, headers=headers, data=data) as response:
                    ...
                    if response.status == 200:
                        ...
                        logger.info(f"声纹识别耗时: {total_elapsed_time:.3f}s")
                        
                        # 相似度阈值检查
                        if score < SPEAKER_VERIFICATION_THRESHOLD:
                            logger.warning(f"声纹识别相似度{score:.3f}低于阈值{SPEAKER_VERIFICATION_THRESHOLD}")
                            return False
                        
                        if speaker_id:
                            logger.info(f"声纹识别成功: {speaker_id} (相似度: {score:.3f})")
                            return True
                        else:
                            logger.warning(f"未识别的说话人ID: {speaker_id}")
                            return True
                    else:
                        logger.error(f"声纹识别API错误: HTTP {response.status}")
                        return None
                        
        except asyncio.TimeoutError:
            elapsed = time.monotonic() - api_start_time
            logger.error(f"声纹识别超时: {elapsed:.3f}s")
            return False
        except Exception as e:
            elapsed = time.monotonic() - api_start_time
...
def pcm_to_wav(pcm_data: bytes) -> bytes:
        ...
        try:
            ...
            logger.info(f"PCM到WAV转换用时: {total_elapsed_time:.3f}s")
